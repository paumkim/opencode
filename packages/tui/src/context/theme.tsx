...
import { Global } from "@term-ai/core/global"
import { Glob } from "@term-ai/core/util/glob"
...
const themeSource: ThemeSource = {
  async discover() {
    ...
    for (let current = process.cwd(); ; current = path.dirname(current)) {
      directories.push(path.join(current, ".term"))
  ...
  init: (props: { mode: "dark" | "light"; source?: ThemeSource }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "term")
        draft.active = typeof active === "string" ? active : "term"
        draft.ready = false
      }),
    )
      ...
      return renderer
        ...
        .then((colors: TerminalColors) => {
          if (!colors.palette[0]) {
            ...
            if (store.active === "system") setStore("active", "term")
            return
          }
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.term, store.mode)
    })
