...
export const configure = (options: ModelOptions) => {
  ...
  const responses = (modelID: string | ModelID, compatibility?: ModelCompatibility.Input) =>
    responsesRoute.with(withOpenAIOptions(modelID, defaults(options))).model({ id: modelID, ...(compatibility !== undefined ? { compatibility } : {}) })
  const chat = (modelID: string | ModelID, compatibility?: ModelCompatibility.Input) =>
    chatRoute.with(withOpenAIOptions(modelID, defaults(options))).model({ id: modelID, ...(compatibility !== undefined ? { compatibility } : {}) })
  return {
    id,
    model: (modelID: string | ModelID, compatibility?: ModelCompatibility.Input) =>
      shouldUseResponsesApi(modelID, options.endpoint) ? responses(modelID, compatibility) : chat(modelID, compatibility),
    responses,
    chat,
    configure,
  }
