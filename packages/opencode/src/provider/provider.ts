...
export const OPENAI_HEADER_TIMEOUT_DEFAULT = 300_000
...
export function googleVertexEndpoint(location: string) {
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make<State>(() =>
      Effect.gen(function* () {
        ...
        const codeium = ProviderV2.ID.make("codeium")
        if (discoveryLoaders[codeium] && providers[codeium] && isProviderAllowed(codeium)) {
          yield* Effect.promise(async () => {
            try {
              const discovered = await discoveryLoaders[codeium]()
              for (const [modelID, model] of Object.entries(discovered)) {
                if (!providers[codeium].models[modelID]) {
                  providers[codeium].models[modelID] = model
                }
              }
            } catch (e) {}
          })
