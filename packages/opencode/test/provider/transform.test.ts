...
describe("ProviderTransform.temperature - Cohere North", () => {
  test("defaults north-mini-code models to 0.2 for coding", () => {
    expect(ProviderTransform.temperature({ id: "cohere/North-Mini-Code-1-0-latest" } as any)).toBe(0.2)
  })
})
...
describe("ProviderTransform.maxOutputTokens", () => {
  test("non-reasoning model with known output cap", () => {
    const model = { ...mockModel, limit: { ...mockModel.limit, output: 8192 }, capabilities: { ...mockModel.capabilities, reasoning: false } }
    expect(ProviderTransform.maxOutputTokens(model)).toBe(8192)
  })

  test("reasoning model with known output cap", () => {
    const model = { ...mockModel, limit: { ...mockModel.limit, output: 8192 }, capabilities: { ...mockModel.capabilities, reasoning: true } }
    expect(ProviderTransform.maxOutputTokens(model)).toBe(8192)
  })

  test("reasoning model with UNKNOWN output cap (0) does not return 0", () => {
    const model = { ...mockModel, limit: { ...mockModel.limit, output: 0 }, capabilities: { ...mockModel.capabilities, reasoning: true } }
    // BUG: previously returned 0, which the OpenAI SDK rejects ("maxOutputTokens must be >= 1")
    expect(ProviderTransform.maxOutputTokens(model)).toBe(32000)
    expect(ProviderTransform.maxOutputTokens(model)).toBeGreaterThan(0)
  })

  test("non-reasoning model with unknown output cap (0) falls back to default", () => {
    const model = { ...mockModel, limit: { ...mockModel.limit, output: 0 }, capabilities: { ...mockModel.capabilities, reasoning: false } }
    expect(ProviderTransform.maxOutputTokens(model)).toBe(32000)
  })

  test("never returns below 1", () => {
    const model = { ...mockModel, limit: { ...mockModel.limit, output: 0 }, capabilities: { ...mockModel.capabilities, reasoning: true } }
    expect(ProviderTransform.maxOutputTokens(model, 0)).toBeGreaterThanOrEqual(1)
  })
})
